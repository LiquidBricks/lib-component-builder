export function buildRegistration({ name, nodes, hash }) {
  ...
  const serviceNodes = serializeServices(nodes.services);
...
function serializeServices(services = { provide: new Map(), require: new Set() }) {
  const provide = serializeNodes(services.provide);
  const require = Array.from(services.require ?? new Set());
